...
When all the objects in the data have been scanned and in some cases, substituted, Yamp outputs the new object tree on the standard output in YAML format. Because YAML maps are unordered, the order of the keys and their corresponding values on output maybe be different from the input footnote:[Order-preservation may happen in a future version, but it's complicated].
...
If the subvariable variable cannot be found for some reason a warning is printed on the standard error and the expansion is the whole dot variable. 
...
The `if` macro renders one value from a choice of two based on whether the condition argument is true. Where true means it's `true` or not `false` or `null`. The `then` argument is returned if so, otherwise the `else` argument. You don't need both `then` and `else` arguments - when the condition requires the missing one, it expands to `null`. The `then` and `else` clauses may have been expanded by the processor prior to the condition evaluation. The condition just serves to choose which of the two expansions are to be used, rather than controlling execution flow. See *Side Effects* below.
...
Yamp can scan the entire clause being processed for variable replacements before it executes builtins and macros. Hence you should not code side-effects via `define` or via `python_eval`. 
The processing may invoke them and then throw away the results. `if` does this for example.  
